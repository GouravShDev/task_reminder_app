...
void main() {
  ...
  group('ToDo', () {
    ...
    test('should call database store method when storeTodo is called',
        () async {
      // act
      await todoDatabaseDataSource.storeToDo(todo);
  ...
  group('Task List', () {
    final TaskList taskList = TaskList(name: 'test');
    test(
        'should call database getAllTodos method when getTodolist is called and return task lists',
        () async {
      // arrange
      await todoDatabaseDataSource.storeTaskList(taskList);
      // act
      final result = await todoDatabaseDataSource.getTaskLists();
      // assert
      expect(result, [
        taskList.copyWith(id: 1),
      ]);
    });
    test(
        'should update todo when store method when storeTodo is called with todo with extisting id',
        () async {
      final TaskList updatedTaskList =
          taskList.copyWith(id: 1, name: 'UpdatedTodo');
      // act
      await todoDatabaseDataSource.storeTaskList(taskList);
      await todoDatabaseDataSource.storeTaskList(updatedTaskList);

      final result = await todoDatabaseDataSource.getTaskLists();

      expect(result, [updatedTaskList]);
    });
  });
}
